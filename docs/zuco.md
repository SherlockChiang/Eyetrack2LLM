...
The current conversion reports predate raw-input hash capture and therefore do not authenticate the 12 analyzed `.mat` files by SHA-256. Reproduction must use the official OSF files named below. A future raw-data conversion should record each local input hash before conversion; this repository does not infer or reconstruct missing hashes from derived files.
