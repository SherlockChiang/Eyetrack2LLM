...
The combined pattern is substantial observable difference in geometry, exposure, feature distributions, and residual dispersion, alongside relatively small external-coefficient versus local-crossfit descriptive conditional-NLL gaps. These gaps combine training-size and fixed-L2 differences and are not transport penalties. The audit does not support a binary declaration that measurement invariance either holds or fails. It does mean the ZuCo transfer uncertainty cannot be uniquely attributed to ineffective supervision; measurement and corpus differences remain a quantitatively supported alternative contributor.
